import { getComponentsChangelog } from "../utils/components.js";
...
2. 在知道用户 antd 版本的情况下，当用户需要实现相关组件功能时判断是否在后续版本中才实现，来决定是否需要升级依赖`,
    ...
    async ({ componentName }) => {
      ...
      if (typeof componentsChangelog === "string")
        return {
          content: [
            {
              type: "text",
      ...
      const currentComponentChangelog =
        componentsChangelog[componentName] ||
        componentsChangelog[componentName.charAt(0).toUpperCase() + componentName.slice(1)];
        ...
        content: [
          {
            ...
            text: currentComponentChangelog
              ? `以下是 ${componentName} 组件的更新日志：
${reduceChangelogContent}`
              : "当前组件没有找到更新日志",
          },
        ],
      };
